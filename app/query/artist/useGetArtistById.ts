...
// Services
import { getArtistById } from "@/app/service/apis/artist.api";

export const useGetDataArtistById = (id: string) => {
  const queryClient = useQueryClient();

  const { data: cachedData, isLoading: isCachedLoading } = useQuery(
    [QUERIES_KEYS.GET_ARTIST, id],
    () => queryClient.getQueryData([QUERIES_KEYS.GET_ARTIST, id]),
    {
      enabled: false,
    },
  );
